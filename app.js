// Config and HTTP requests
const { get, post } = require('snekfetch');
const cfg = require('./config');

// Database stuff
const data = './data.sqlite';
...
app.get('/verify', (req, res) => {
  ...
  if (discord && twitch) {
    open(data).then(async db => {
      const users = await db.run('SELECT * FROM users WHERE id = (?)', discord.id);
      if (users.length) return;
      await db.run('INSERT INTO users (id, name, subbed) VALUES (?, ?, ?)', discord.id, twitch.name, twitch.subbed);
      await db.close();
    });
  }
  ...
  res.send([
    discord
      ? `Logged in as ${discord.tag} (${discord.id})`
      : '<a href="/verify/discord/login">Login to Discord</a>',
    twitch
      ? `Logged in as ${twitch.name}`
      : '<a href="/verify/twitch/login">Login to Twitch</a>',
    date
      ? `Subbed since ${months[date.getMonth()]} ${ordinal(date.getDate() + 1)}, ${date.getFullYear()}`
      : twitch
